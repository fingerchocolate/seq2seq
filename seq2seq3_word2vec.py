#$ python3 seq2seq3_word2vec.py --gpu=0 original_train_prepro.txt simple_train_prepro.txt \wikiextractor/wiki.model \-s snap_model
...
class Seq2seq(chainer.Chain):
    ...
    def __init__(self, n_layers, n_source_vocab, n_target_vocab, n_units, word2vec):
        super(Seq2seq, self).__init__()
        with self.init_scope():
            self.embed_x = L.EmbedID(n_source_vocab, n_units, initialW=word2vec.wv.vectors)##
            self.embed_y = L.EmbedID(n_target_vocab, n_units, initialW=word2vec.wv.vectors)##
...
def load_vocabulary(vocab):##
    # +2 for UNK and EOS##
    word_ids = {w : i + 2 for i, w in enumerate(vocab)}##
    word_ids['<UNK>'] = 0##
    word_ids['<EOS>'] = 1##
    return word_ids##
...
def main():
    ...
    parser.add_argument('model_word2vec', help='model ward2vec')##
...
#    parser.add_argument('--validation-interval', type=int, default=4000,
    ...
    word2vec = gensim.models.Word2Vec.load(args.model_word2vec)##
    i2w = word2vec.wv.index2word##
    source_ids = load_vocabulary(i2w)
    target_ids = load_vocabulary(i2w)
    ...
    # Setup model
    model = Seq2seq(args.layer, len(source_ids), len(target_ids), args.unit, word2vec)
    if args.gpu >= 0:
    ...
    if args.validation_source and args.validation_target:
        if args.use_dataset_api: ##
            ...
            test_data = load_data_using_dataset_api( ##
                source_ids, args.validation_source, ##
                target_ids, args.validation_target, ##
                _filter_func,##
            ) ##
        else:
